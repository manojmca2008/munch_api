...
class SearchUrlsMobile {
    ...
    private function setPriceDealsAcceptsOrderFq($req) 
    {
        ...
        if ($req['deals'] > 0) 
        {
            $this->deals_fq = SearchHelpers::getSolrDealsFq($req['tab']);
        }
    ...
    private function getCommonFqFilters($req){
        ...
        if($req['orn'] == 1){
            $commonFq .= SearchHelpers::getDiscoverTimeFq($req['day'], $req['curr_time']);
        }
    ...
    private function getDineinFqFilters($req) {
        ...
        if( !empty($req['sdate'])){
            $solrfq .= SearchHelpers::getDineinTimeFqMob($this->day, $this->time);
        }
        return $solrfq;
    }
    ...
    private function getReserveFqFilters($req) {
        ...
        //$solrfq .= '&fq=meals_arr:"' . $req['subtab'] . '"';
        if($req['all'] == '0'){
            $solrfq .= SearchHelpers::getReservationTimeFqMob($this->day, $this->time);
        }
    ...
    private function getPaaCommonFq($req){
        ...
        if($req['orn'] == 1){
            $solrfq .= SearchHelpers::getDiscoverTimeFq($req['day'], $req['curr_time']);
        }
        return $solrfq;
    }
